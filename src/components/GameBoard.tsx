...
export default function GameBoard({ theme, settings, cardSetType, customCards, onHome }: GameBoardProps) {
  ...
  const { cards, moves, matchedCount, isGameOver, flipCard, startGame } = useMemoryGame(pairCount, soundOn, speechOn, liveSettings.flipDuration, liveSettings.speechRate, liveSettings.customVoiceEnabled !== false, liveSettings.sfxMode || "builtin", liveSettings.elevenLabsEffectsEnabled === true);
